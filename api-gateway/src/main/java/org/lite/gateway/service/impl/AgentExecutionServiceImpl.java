...
public class AgentExecutionServiceImpl implements AgentExecutionService {
                ...
                return Mono.zip(
                                ...
                                agentTaskRepository.findById(taskId)).flatMap(tuple -> {
                                        ...
                                        log.info("Task {} state: enabled={}, cronExpression={}, autoExecute={}, executionTrigger={}",
                                                        taskId, task.isEnabled(), task.getCronExpression(),
                                                        task.isAutoExecute(),
                                        ...
                                        if (!task.isReadyToExecute()) {
                                                ...
                                                return auditLogHelper.logDetailedEvent(
                                                                ...
                                                                .doOnError(auditError -> log.error(
                                                                                "Failed to log audit event (task not ready): {}",
                                        ...
                                        if (!task.isExecutionTriggerValid()) {
                                                log.error("Task {} has invalid execution trigger configuration: {}",
                                                                taskId,
                                                                task.getExecutionTrigger());
                                                ...
                                                return auditLogHelper.logDetailedEvent(
                                                                ...
                                                                AuditResultType.DENIED)
                                                                .doOnError(auditError -> log.error(
                                                                                "Failed to log audit event (invalid trigger): {}",
                                                                                auditError.getMessage(), auditError))
                                                                .onErrorResume(auditError -> Mono.empty()) // Don't fail
                                                                                                           // if audit
                                                                                                           // logging
                                        ...
                                        if (task.getExecutionTrigger() == ExecutionTrigger.CRON
                                                        && !task.isAutoExecute()) {
                                                log.error("Task {} is configured for CRON trigger but autoExecute is false",
                                                                taskId);
                                                ...
                                                return auditLogHelper.logDetailedEvent(
                                                                ...
                                                                AuditResultType.DENIED)
                                                                .doOnError(auditError -> log.error(
                                                                                "Failed to log audit event (CRON autoExecute): {}",
                                                                                auditError.getMessage(), auditError))
                                                                .onErrorResume(auditError -> Mono.empty()) // Don't fail
                                                                                                           // if audit
                                                                                                           // logging
                                        ...
                                        if (!agent.canExecute()) {
                                                ...
                                                return auditLogHelper.logDetailedEvent(
                                                                ...
                                                                AuditResultType.DENIED)
                                                                .doOnError(auditError -> log.error(
                                                                                "Failed to log audit event (agent not ready): {}",
                                                                                auditError.getMessage(), auditError))
                                                                .onErrorResume(auditError -> Mono.empty()) // Don't fail
                                                                                                           // if audit
                                                                                                           // logging
                                        ...
                                        return agentExecutionRepository.save(execution)
                                                        .flatMap(savedExecution -> {
                                                                // Use saved execution from now on to ensure we have the
                                                                // persisted version
                                                                return executionQueueService
                                                                                .addToQueue(executionId, agentId,
                                                                                ...
                                                                                .then(executionQueueService
                                                                                                .markAsStartedAndRemove(
                                                                                                                executionId))
                                                                                .then(sendExecutionStartedUpdate(
                                                                                                savedExecution, agent,
                                                                                                task))
                                                                                .doOnError(
                                                                                                updateError -> log
                                                                                                                .error("Failed to send execution started update: {}",
                                                                                                                                updateError.getMessage(),
                                                                                                                                updateError))
                                                                                .onErrorResume(updateError -> Mono
                                                                                                .empty()) // Don't fail
                                                                                                          // if update
                                                                                                          // sending
                                                                                                          // fails
                                                                                ...
                                                                                .then(Mono.defer(() -> {
                                                                                        ...
                                                                                        auditContext.put("executionId",
                                                                                                        executionId);
                                                                                        auditContext.put("agentId",
                                                                                                        agentId);
                                                                                        auditContext.put("agentName",
                                                                                                        agent.getName());
                                                                                        auditContext.put("taskId",
                                                                                                        taskId);
                                                                                        auditContext.put("taskName",
                                                                                                        task.getName());
                                                                                        auditContext.put("teamId",
                                                                                                        teamId);
                                                                                        auditContext.put("executedBy",
                                                                                                        executedBy);
                                                                                        auditContext.put(
                                                                                                        "executionType",
                                                                                        ...
                                                                                        auditContext.put(
                                                                                                        "startTimestamp",
                                                                                                        savedExecution.getStartedAt()
                                                                                                                        .toString());

                                                                                        // Log audit event as part of
                                                                                        // the reactive chain (preserves
                                                                                        // context)
                                                                                        // Use doOnSuccess/doOnError to
                                                                                        // log audit as side effect, but
                                                                                        // always return
                                                                                        // execution
                                                                                ...
                                                                                .onErrorResume(error -> {
                                                                                        // ERROR CASE - Only update
                                                                                        // execution status
                                                                                        log.error("Task execution failed: {}",
                                                                                                        error.getMessage());

                                                                                        // Reload execution from
                                                                                        // database to ensure we have
                                                                                        // the latest version
                                                                                                        ...
                                                                                                        .flatMap(executionToUpdate -> {
                                                                                                                ...
                                                                                                                return auditLogHelper
                                                                                                                                ...
                                                                                                                                .then(agentExecutionRepository
                                                                                                                                                .save(executionToUpdate))
                                                                                                                                .flatMap(saved -> {
                                                                                                                                        try {
                                                                                                                                                return sendExecutionFailedUpdate(
                                                                                                                                                                saved,
                                                                                                                                                                agent,
                                                                                                                                                                task,
                                                                                                                                                                error.getMessage());
                                ...
                                .onErrorResume(error -> {
                                        ...
                                        return auditLogHelper.logDetailedEvent(
                                                        ...
                                                        AuditResourceType.AGENT_TASK_EXECUTION,
                                                        taskId,
                                                        String.format("Task execution initiation failed: %s",
                                                                        error.getMessage()),
                                                        errorContext,
                                                        null,
                                                        null,
                                                        AuditResultType.FAILED)
                                                        .doOnError(auditError -> log.error(
                                                                        "Failed to log audit event (initiation failed): {}",
                                                                        auditError.getMessage(), auditError))
        ...
        public Flux<AgentExecution> getRecentExecutions(String teamId, int limit) {
                return agentExecutionRepository.findByTeamIdOrderByCreatedAtDesc(teamId)
                                ...
                                .doOnError(error -> log.error("Error fetching recent executions for team {}: {}",
                                                teamId,
                                                error.getMessage()));
        }
        ...
        private Mono<Void> sendExecutionStartedUpdate(AgentExecution execution, Agent agent, AgentTask task) {
                if (execution == null) {
                        ...
                        return Mono.error(new IllegalStateException(
                                        "Execution is null when trying to send started update"));
                }
        ...
        private Mono<Void> sendExecutionFailedUpdate(AgentExecution execution, Agent agent, AgentTask task,
                ...
                if (execution == null) {
                        ...
                        return Mono.error(new IllegalStateException(
                                        "Execution is null when trying to send failed update"));
                }
        ...
        private int getTotalSteps(AgentTask task) {
                try {
                        ...
                        log.warn("Failed to extract step count from task {}, using default: {}", task.getId(),
                                        e.getMessage());
                }
