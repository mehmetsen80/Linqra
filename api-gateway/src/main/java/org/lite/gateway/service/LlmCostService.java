...
public interface LlmCostService {
     ...
     */
    Mono<Double> calculateCostForMonth(String teamId, String model, long promptTokens, long completionTokens, YearMonth yearMonth);
    ...
    /**
     * Initialize current month's pricing snapshots for a team from the static pricing table
    ...
    /**
     * Backfill cost calculation for existing workflow executions that are missing cost data
     * This will:
    ...
    /**
     * Advanced backfill that extracts token usage from response data for executions missing tokenUsage
     * This is for Cohere (and other providers) where token data exists in the response but wasn't extracted to metadata
     * 
     * @param teamId Optional team ID to limit backfill (null for all teams)
     * @param dryRun If true, only logs what would be updated without making changes
     * @return Mono with count of updated executions
