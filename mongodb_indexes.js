...
// 1. Team Version Index (unique constraint)
db.team_chunk_keys.createIndex(
  { "teamId": 1, "version": 1 },
  {
    "name": "team_version_idx",
    "unique": true,
    "background": true
  }
);
...
// 2. Team Timestamp Index (primary query pattern)
db.audit_logs.createIndex(
...
// 5. Document Timestamp Index
db.audit_logs.createIndex(
  { "documentId": 1, "timestamp": -1 },
  {
    "name": "document_timestamp_idx",
    "background": true
  }
);

// 6. Team Event Type Timestamp Index (compound)
db.audit_logs.createIndex(
  { "teamId": 1, "eventType": 1, "timestamp": -1 },
  {
    "name": "team_event_timestamp_idx",
    "background": true
  }
);

// 7. Team Result Timestamp Index (compound)
db.audit_logs.createIndex(
  { "teamId": 1, "result": 1, "timestamp": -1 },
  {
    "name": "team_result_timestamp_idx",
    "background": true
  }
);

// 8. Team User Timestamp Index (compound)
db.audit_logs.createIndex(
  { "teamId": 1, "userId": 1, "timestamp": -1 },
  {
    "name": "team_user_timestamp_idx",
    "background": true
  }
);

// 9. Not Archived Timestamp Index (sparse - for archival queries)
db.audit_logs.createIndex(
  { "archivedAt": 1, "timestamp": 1 },
  {
    "name": "not_archived_timestamp_idx",
    "sparse": true,
    "background": true
  }
);

// 10. Collection ID Index (for filtering by collection)
db.audit_logs.createIndex(
  { "collectionId": 1, "timestamp": -1 },
  {
    "name": "collection_timestamp_idx",
    "background": true
  }
);
