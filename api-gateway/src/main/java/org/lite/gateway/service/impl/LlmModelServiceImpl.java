...
public class LlmModelServiceImpl implements LlmModelService {
        ...
        public Mono<LlmModel> createModel(LlmModel model) {
                ...
                return llmModelRepository.save(model)
                                .flatMap(saved -> llmCostService.refreshPricingCache()
                                                .doOnSuccess(v -> log.info(
                                                                "✅ Refreshed pricing cache after creating model: {}",
                                                                saved.getModelName()))
                                                .thenReturn(saved));
        }
        ...
        public Mono<LlmModel> updateModel(String id, LlmModel model) {
                return llmModelRepository.findById(id)
                                ...
                                .flatMap(saved -> llmCostService.refreshPricingCache()
                                                .doOnSuccess(v -> log.info(
                                                                "✅ Refreshed pricing cache after updating model: {}",
                                                                saved.getModelName()))
                                                .thenReturn(saved));
        }
        ...
        public Mono<Void> deleteModel(String id) {
                return llmModelRepository.findById(id)
                                .flatMap(model -> llmModelRepository.deleteById(id)
                                                .then(llmCostService.refreshPricingCache())
                                                .doOnSuccess(v -> log.info(
                                                                "✅ Refreshed pricing cache after deleting model: {}",
                                                                model.getModelName())));
        }
        ...
        public void onApplicationReady() {
                ...
                initializeDefaultModels()
                                ...
                                .doOnError(error -> log.error("Error during LLM models initialization: {}",
                                                error.getMessage()))
                                .subscribe();
        }
                ...
                return cleanupAllDuplicates()
                                ...
                                .flatMap(count -> {
                                        ...
                                        if (count == expectedCount) {
                                                ...
                                                log.info("✅ All {} default models exist, skipping initialization",
                                                                expectedCount);
                                                return Mono.empty();
                                        }
                                ...
                                .doOnError(error -> log.error("❌ Error during models initialization: {}",
                                                error.getMessage()));
        }
        ...
        private Mono<Void> initializeDefaultModelsInternal() {
                ...
                return Flux.fromIterable(getDefaultModels())
                                .concatMap(defaultModel -> llmModelRepository
                                                .findByModelName(defaultModel.getModelName())
                                                .onErrorResume(org.springframework.dao.IncorrectResultSizeDataAccessException.class,
                                                                error -> {
                                                                        // Duplicates exist - skip this model, cleanup
                                                                        // already ran
                                                                        log.warn("Found duplicate models for {}, skipping (cleanup will handle this)",
                                                                                        defaultModel.getModelName());
                                                                        return Mono.empty();
                                                                })
                                                .flatMap(existing -> {
                                                        log.debug("Model {} already exists, skipping",
                                                                        defaultModel.getModelName());
                                                        return Mono.empty();
                                                })
                                                .switchIfEmpty(
                                                                llmModelRepository.save(defaultModel)
                                                                                .onErrorResume(org.springframework.dao.DuplicateKeyException.class,
                                                                                                error -> {
                                                                                                        ...
                                                                                                        // between check
                                                                                                        // and save
                                                                                                        log.debug("Model {} was created by another thread, skipping",
                                                                                                                        defaultModel.getModelName());
                                                                                                        return Mono.empty();
                                                                                                })
        ...
        private List<LlmModel> getDefaultModels() {
                ...
                models.add(createModel("text-embedding-004", "Gemini Text Embedding 004 (Legacy)", "gemini",
                                "embedding",
                ...
                models.add(createModel("claude-sonnet-4-5", "Claude Sonnet 4.5 (Latest)", "anthropic", "chat",
                                3.00, 15.00, true,
                                "Latest smartest model for complex agents and coding (claude-sonnet-4-5-20250929)",
                ...
                models.add(createModel("claude-haiku-4-5", "Claude Haiku 4.5 (Latest)", "anthropic", "chat",
                                0.80, 4.00, true,
                                "Fastest model with near-frontier intelligence (claude-haiku-4-5-20251001)",
                                "https://api.anthropic.com/v1/messages", null, 200_000, now));
                models.add(createModel("claude-opus-4-1", "Claude Opus 4.1 (Premium)", "anthropic", "chat",
                                15.00, 75.00, true,
                                "Premium model for specialized reasoning tasks. Cost: $15/$75 per 1M tokens (claude-opus-4-1-20250805)",
                                "https://api.anthropic.com/v1/messages", null, 200_000, now));
                ...
                models.add(createModel("command-a-03-2025", "Command A", "cohere", "chat",
                                2.50, 10.00, true,
                                "Most performant Cohere model (111B params, 256K context) for enterprise agents, RAG, and multilingual tasks",
                                "https://api.cohere.ai/v2/chat", null, 256_000, now));
                models.add(createModel("command-r-plus-08-2024", "Command R+", "cohere", "chat",
                                2.50, 10.00, true,
                                "Optimized for conversational interaction and long-context tasks (128K context) with 50% higher throughput. Excellent for complex RAG and multi-step tool use",
                                "https://api.cohere.ai/v2/chat", null, 128_000, now));
                models.add(createModel("command-r-08-2024", "Command R", "cohere", "chat",
                                0.15, 0.60, true,
                                "Scalable model balancing high performance with strong accuracy (128K context) - 50% higher throughput. Optimized for RAG, tool use, and multilingual tasks",
                                "https://api.cohere.ai/v2/chat", null, 128_000, now));
                models.add(createModel("command-r7b-12-2024", "Command R7B", "cohere", "chat",
                                0.0375, 0.15, true,
                                "Smallest and fastest R family model (7B params, 128K context) - Ideal for high throughput, chatbots, and on-device inference",
                                "https://api.cohere.ai/v2/chat", null, 128_000, now));
                ...
                models.add(createModel("embed-v4.0", "Embed v4", "cohere", "embedding",
                                0.120, 0.0, true,
                                "Latest Cohere embedding model with configurable dimensions (256/512/1024/1536, default 1536) and 128k context. Supports text, images, and PDFs",
                ...
                models.add(createModel("embed-multilingual-v3.0", "Embed Multilingual v3", "cohere", "embedding",
                                0.100, 0.0, true,
                                "Multilingual embedding model supporting multiple languages. Supports text and images",
                                "https://api.cohere.ai/v1/embed", 1024, 65_536, now));
                models.add(createModel("embed-english-light-v3.0", "Embed English Light v3", "cohere", "embedding",
                                0.100, 0.0, true,
                                "Smaller, faster version of embed-english-v3.0. Almost as capable, but a lot faster. English only. Supports text and images",
                                "https://api.cohere.ai/v1/embed", 384, 65_536, now));
                models.add(createModel("embed-multilingual-light-v3.0", "Embed Multilingual Light v3", "cohere",
                                "embedding",
                                0.100, 0.0, true,
                                "Smaller, faster version of embed-multilingual-v3.0. Almost as capable, but a lot faster. Supports multiple languages and images",
                                "https://api.cohere.ai/v1/embed", 384, 65_536, now));
                ...
                models.add(createModel("nomic-embed-text", "Ollama Nomic Embed Text", "ollama", "ollama-embed",
                                0.0, 0.0, true, "768 dim, 2k context window (Safe)",
                                "http://localhost:11434/api/embeddings", 768, 2048, now));
                models.add(createModel("mxbai-embed-large", "Ollama Mxbai Embed Large", "ollama", "ollama-embed",
                                0.0, 0.0, true, "1024 dim, State-of-the-art retrieval",
                                "http://localhost:11434/api/embeddings", 1024, 512, now));
                models.add(createModel("snowflake-arctic-embed", "Ollama Snowflake Arctic", "ollama", "ollama-embed",
                                0.0, 0.0, true, "1024 dim, Enterprise grade",
                                "http://localhost:11434/api/embeddings", 1024, 512, now));
                models.add(createModel("all-minilm", "Ollama All-MiniLM", "ollama", "ollama-embed",
                                0.0, 0.0, true, "384 dim, Fast and efficient",
                                "http://localhost:11434/api/embeddings", 384, 256, now));
        ...
        private LlmModel createModel(String modelName, String displayName, String provider, String category,
                        ...
                        String description, String endpoint, Integer dimensions, Integer contextWindowTokens,
                        LocalDateTime now) {
                                ...
                                .flatMapMany(allModels -> {
                                        ...
                                        java.util.Map<String, java.util.List<LlmModel>> grouped = allModels.stream()
                                                        .collect(java.util.stream.Collectors
                                                                        .groupingBy(LlmModel::getModelName));
                                        ...
                                        return Flux.fromIterable(grouped.entrySet())
                                                        .concatMap(entry -> {
                                                                ...
                                                                if (models.size() <= 1) {
                                                                        log.debug("Model {} has no duplicates, skipping",
                                                                                        modelName);
                                                                        return Flux.empty();
                                                                }

                                                                log.info("Found {} duplicates for model {}, cleaning up",
                                                                                models.size(), modelName);

                                                                // Keep first, delete rest sequentially
                                                                return Flux.fromIterable(
                                                                                models.subList(1, models.size()))
                                                                                .concatMap(duplicate -> {
                                                                                        log.info("Deleting duplicate {} (ID: {})",
                                                                                                        modelName,
                                                                                                        duplicate.getId());
                                                                                        return llmModelRepository
                                                                                                        .deleteById(duplicate
                                                                                                                        .getId())
