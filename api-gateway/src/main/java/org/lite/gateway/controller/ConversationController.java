...
public class ConversationController {
    ...
    private Mono<Boolean> checkAccessAuthorization(String conversationId, String username, String teamId, String userId) {
        return conversationService.getConversationById(conversationId)
                .flatMap(conversation -> {
                    // Check if user owns the conversation (by username)
                    if (conversation.getUsername() != null && conversation.getUsername().equals(username)) {
                ...
                .flatMap(user -> teamContextService.getTeamFromContext(exchange)
                        .flatMap(teamId -> {
                            ...
                            return chatExecutionService.executeChat(linqRequest)
                                    ...
                                    .onErrorResume(error -> {
                                        log.error("Error starting conversation: {}", error.getMessage());
    ...
    public Mono<ResponseEntity<Map<String, Object>>> sendMessage(
        ...
        return userContextService.getCurrentUsername(exchange)
                .flatMap(userService::findByUsername)
                .flatMap(user -> teamContextService.getTeamFromContext(exchange)
                        .flatMap(teamId -> checkAccessAuthorization(conversationId, user.getUsername(), teamId, user.getId())
                                .filter(hasAccess -> hasAccess)
                                .switchIfEmpty(Mono.error(new RuntimeException("Unauthorized access to conversation")))
                                .flatMap(hasAccess -> conversationService.getConversationById(conversationId)
                                        .flatMap(conversation -> {
                                            String message = (String) requestBody.getOrDefault("message", "");
                                            ...
                                            return conversationService.getMessagesByConversationId(conversationId)
                                                    ...
                                                    .flatMap(historyMessages -> {
                                                        ...
                                                        LinqRequest.Query.ChatConversation chat = new LinqRequest.Query.ChatConversation();
                                                        chat.setAssistantId(conversation.getAssistantId());
                                                        chat.setMessage(message);
                                                        ...
                                                        for (ConversationMessage msg : historyMessages) {
                                                            ...
                                                            chatMsg.setRole(msg.getRole().toLowerCase());
                ...
                .flatMap(user -> teamContextService.getTeamFromContext(exchange)
                        .flatMap(teamId -> checkAccessAuthorization(conversationId, user.getUsername(), teamId, user.getId())
                                ...
                                .map(hasAccess -> {
                                    ...
                                    Flux<ConversationMessage> messages = conversationService.getMessagesByConversationId(conversationId, pageable);
        ...
        return userContextService.getCurrentUsername(exchange)
                ...
                .flatMap(user -> teamContextService.getTeamFromContext(exchange)
                        .map(teamId -> {
                            ...
                            if (assistantId != null) {
                                // Filter by both assistantId and username to show only current user's conversations
                                conversations = conversationService.listConversationsByAssistantAndUser(
                                        assistantId, user.getUsername(), pageable);
                            } else {
                                conversations = conversationService.listConversationsByTeam(teamId, pageable);
                            }
        ...
        return userContextService.getCurrentUsername(exchange)
                ...
                .flatMap(user -> teamContextService.getTeamFromContext(exchange)
                        .flatMap(teamId -> checkAccessAuthorization(conversationId, user.getUsername(), teamId, user.getId())
                                .filter(hasAccess -> hasAccess)
                                .switchIfEmpty(Mono.error(new RuntimeException("Unauthorized access to conversation")))
                                .flatMap(hasAccess -> conversationService.deleteConversation(conversationId)
