...
/**
 * Implementation of AgentAuthContextService for handling agent and task
 * authorization.
 * 
 * This service provides centralized authorization logic, checking if users have
 * proper
 * permissions (SUPER_ADMIN or team ADMIN) to perform operations on agents and
 * tasks.
 */
@Service
...
public class AgentAuthContextServiceImpl implements AgentAuthContextService {
    ...
    public Mono<AgentAuthContext> checkAgentAuthorization(String agentId, ServerWebExchange exchange) {
        ...
        return userContextService.getCurrentUsername(exchange)
                ...
                .doOnError(
                        error -> log.warn("Agent authorization failed for agent {}: {}", agentId, error.getMessage()));
    }
     ...
     * @return Mono containing AgentAuthContext if authorized, or error if not
     *         authorized
     */
    private Mono<AgentAuthContext> performAuthorizationCheck(User user, String agentId, String taskId, String teamId,
            String resourceType) {
        // Check authorization: SUPER_ADMIN or team ADMIN
        if (user.getRoles().contains("SUPER_ADMIN")) {
            log.info("User {} authorized as SUPER_ADMIN for {} {}",
        ...
        return teamService.hasRole(teamId, user.getId(), "ADMIN")
                .map(isAdmin -> {
                    ...
                    log.info("User {} authorized as team ADMIN for {} {} in team {}",
                            user.getUsername(), resourceType,
