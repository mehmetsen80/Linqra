...
public class AgentTaskServiceImpl implements AgentTaskService {
        ...
        public Mono<AgentTask> createTask(AgentTask task) {
                log.info("Creating task '{}' for agent {} (type: {})", task.getName(), task.getAgentId(),
                                task.getTaskType());
                ...
                if (!task.isExecutionTriggerValid()) {
                        return Mono.error(new IllegalArgumentException(
                                        String.format("Invalid execution trigger configuration for task: %s",
                                                        task.getName())));
                }
                ...
                return agentRepository.findById(task.getAgentId())
                                ...
                                .flatMap(agent -> {
                                        ...
                                        if (task.getCronExpression() != null
                                                ...
                                                return Mono.just(cronDescriptionService
                                                                .getCronDescription(task.getCronExpression()))
                                                                .flatMap(description -> {
                                                                        task.setCronDescription(description);
                                                                        log.info("Generated cron description for task '{}': {}",
                                ...
                                .flatMap(savedTask -> {
                                        ...
                                        return agentRepository.findById(savedTask.getAgentId())
                                                        .flatMap(agent -> saveInitialTaskVersion(savedTask,
                                                                        agent.getTeamId()))
                                                        .thenReturn(savedTask);
                                })
                                .doOnSuccess(savedTask -> log.info(
                                                "Task '{}' created successfully with ID: {} and version 1 (type: {})",
                                                savedTask.getName(), savedTask.getId(), savedTask.getTaskType()))
                                .doOnError(error -> log.error("Failed to create task '{}': {}", task.getName(),
                                                error.getMessage()));
        }
        ...
        public Mono<Boolean> deleteTask(String taskId) {
                ...
                return getTaskById(taskId)
                                ...
                                .doOnError(error -> log.error("Failed to delete task {}: {}", taskId,
                                                error.getMessage()));
        }
        ...
        public Mono<AgentTask> setTaskEnabled(String taskId, boolean enabled, String updatedBy) {
                ...
                return getTaskById(taskId)
                                ...
                                .doOnSuccess(updatedTask -> log.info("Task {} enabled={} successfully by user {}",
                                                taskId, enabled, updatedBy))
                                .doOnError(error -> log.error("Failed to set task {} enabled={} by user {}: {}", taskId,
                                                enabled, updatedBy, error.getMessage()));
        }
        ...
        // NOTE: getTasksByAgentAndStatus method removed - task status now managed by
        // AgentExecution
                                        ...
                                        return agentExecutionService.getTaskExecutionHistory(taskId, Integer.MAX_VALUE)
                                                        ...
                                                        .map(executions -> {
                                                                ...
                                                                long successfulExecutions = executions.stream()
                                                                                .filter(e -> e.getResult() != null && e
                                                                                                .getResult() == ExecutionResult.SUCCESS)
                                                                                .count();
                                                                long failedExecutions = executions.stream()
                                                                                .filter(e -> e.getResult() != null && e
                                                                                                .getResult() == ExecutionResult.FAILURE)
                                                                                .count();
                                                                ...
                                                                stats.put("taskName", task.getName());
                                                                stats.put("enabled", task.isEnabled());
                                                                stats.put("taskType", task.getTaskType().toString());
                                                                stats.put("totalExecutions", totalExecutions);
                                                                stats.put("successfulExecutions", successfulExecutions);
                                                                stats.put("failedExecutions", failedExecutions);
                                                                stats.put("successRate", Math.round(successRate * 100.0)
                                                                                / 100.0);
                                                                stats.put("averageExecutionTime",
                                                                                Math.round(averageExecutionTime * 100.0)
                                                                                                / 100.0);
                                                                stats.put("lastExecuted", lastExecuted);
                                                                return stats;
                                                        });
                                });
        }
