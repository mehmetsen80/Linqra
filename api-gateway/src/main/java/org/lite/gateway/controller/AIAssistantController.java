...
public class AIAssistantController {
        ...
        return userContextService.getCurrentUsername(exchange)
                ...
                .flatMapMany(user -> {
                    ...
                    return teamService.hasRole(teamId, user.getId(), "ADMIN")
                            ...
                            .switchIfEmpty(Mono.error(new RuntimeException("Admin access required for team " + teamId)))
                            .flatMapMany(hasRole -> aiAssistantService.getAssistantsByTeam(teamId));
                })
                .onErrorResume(error -> {
                    log.warn("Authorization failed for getAssistantsByTeam {}: {}", teamId, error.getMessage());
                    return Flux.empty();
                });
    }
                ...
                .flatMap(assistant -> {
                    return userContextService.getCurrentUsername(exchange)
                            ...
                            .flatMap(user -> {
                                // Check authorization: SUPER_ADMIN or team ADMIN
                                if (user.getRoles().contains("SUPER_ADMIN")) {
                                    return Mono.just(ResponseEntity.ok((Object) assistant));
                                }
                                
                                return teamService.hasRole(assistant.getTeamId(), user.getId(), "ADMIN")
                                        .filter(hasRole -> hasRole)
                                        .switchIfEmpty(Mono.error(new RuntimeException("Admin access required")))
                                        .map(hasRole -> ResponseEntity.ok((Object) assistant));
                            });
                })
                .onErrorResume(error -> {
                    log.warn("Authorization failed for getAssistant {}: {}", assistantId, error.getMessage());
                ...
                .flatMap(teamId -> {
                    ...
                    return userContextService.getCurrentUsername(exchange)
                            ...
                            .flatMap(user -> {
                                // Check authorization: SUPER_ADMIN or team ADMIN
                                if (user.getRoles().contains("SUPER_ADMIN")) {
                                    return aiAssistantService.createAssistant(assistant, teamId, user.getUsername());
                                }
                                
                                return teamService.hasRole(teamId, user.getId(), "ADMIN")
                                        .filter(hasRole -> hasRole)
                                        .switchIfEmpty(Mono.error(new RuntimeException("Admin access required for team " + teamId)))
                                        .flatMap(hasRole -> aiAssistantService.createAssistant(assistant, teamId, user.getUsername()));
                            });
                })
                ...
                .flatMap(existingAssistant -> {
                    return userContextService.getCurrentUsername(exchange)
                            ...
                            .flatMap(user -> {
                                // Check authorization: SUPER_ADMIN or team ADMIN
                                if (user.getRoles().contains("SUPER_ADMIN")) {
                                    return aiAssistantService.updateAssistant(assistantId, assistantUpdates, user.getUsername());
                                }
                                
                                return teamService.hasRole(existingAssistant.getTeamId(), user.getId(), "ADMIN")
                                        .filter(hasRole -> hasRole)
                                        .switchIfEmpty(Mono.error(new RuntimeException("Admin access required")))
                                        .flatMap(hasRole -> aiAssistantService.updateAssistant(assistantId, assistantUpdates, user.getUsername()));
                            });
                })
                .map(updatedAssistant -> ResponseEntity.ok((Object) updatedAssistant))
                .onErrorResume(error -> {
                    log.warn("Failed to update AI Assistant {}: {}", assistantId, error.getMessage());
                    ErrorResponse errorResponse = ErrorResponse.fromErrorCode(
                            ErrorCode.FORBIDDEN,
                            error.getMessage(),
                            HttpStatus.FORBIDDEN.value()
    ...
    public Mono<ResponseEntity<Object>> deleteAssistant(
        ...
        return aiAssistantService.getAssistantById(assistantId)
                    ...
                    return teamContextService.getTeamFromContext(exchange)
                            .flatMap(teamId -> {
                                ...
                                if (!teamId.equals(existingAssistant.getTeamId())) {
                                    return Mono.error(new RuntimeException("Team mismatch"));
                                }
                                ...
                                return userContextService.getCurrentUsername(exchange)
                                        ...
                                        .flatMap(user -> {
                                            // Check authorization: SUPER_ADMIN or team ADMIN
                                            if (user.getRoles().contains("SUPER_ADMIN")) {
                                                return aiAssistantService.deleteAssistant(assistantId, teamId);
                                            }
                                            
                                            return teamService.hasRole(teamId, user.getId(), "ADMIN")
                                                    .filter(hasRole -> hasRole)
                                                    .switchIfEmpty(Mono.error(new RuntimeException("Admin access required")))
                                                    .flatMap(hasRole -> aiAssistantService.deleteAssistant(assistantId, teamId));
                                        });
                            });
                })
                ...
                .onErrorResume(error -> {
                    log.error("Failed to delete AI Assistant {}: {}", assistantId, error.getMessage());
                    ErrorResponse errorResponse = ErrorResponse.fromErrorCode(
                ...
                .flatMap(existingAssistant -> {
                    return userContextService.getCurrentUsername(exchange)
                            ...
                            .flatMap(user -> {
                                // Check authorization: SUPER_ADMIN or team ADMIN
                                if (user.getRoles().contains("SUPER_ADMIN")) {
                                    return aiAssistantService.updateAccessControl(assistantId, accessControl, user.getUsername());
                                }
                                
                                return teamService.hasRole(existingAssistant.getTeamId(), user.getId(), "ADMIN")
                                        .filter(hasRole -> hasRole)
                                        .switchIfEmpty(Mono.error(new RuntimeException("Admin access required")))
                                        .flatMap(hasRole -> aiAssistantService.updateAccessControl(assistantId, accessControl, user.getUsername()));
                            });
                })
                .map(updatedAssistant -> ResponseEntity.ok((Object) updatedAssistant))
                .onErrorResume(error -> {
                    log.warn("Failed to update access control for AI Assistant {}: {}", assistantId, error.getMessage());
                    ErrorResponse errorResponse = ErrorResponse.fromErrorCode(
                            ErrorCode.FORBIDDEN,
                            error.getMessage(),
                            HttpStatus.FORBIDDEN.value()
                ...
                .flatMap(existingAssistant -> {
                    return userContextService.getCurrentUsername(exchange)
                            ...
                            .flatMap(user -> {
                                // Check authorization: SUPER_ADMIN or team ADMIN
                                if (user.getRoles().contains("SUPER_ADMIN")) {
                                    return aiAssistantService.generatePublicApiKey(assistantId, user.getUsername());
                                }
                                
                                return teamService.hasRole(existingAssistant.getTeamId(), user.getId(), "ADMIN")
                                        .filter(hasRole -> hasRole)
                                        .switchIfEmpty(Mono.error(new RuntimeException("Admin access required")))
                                        .flatMap(hasRole -> aiAssistantService.generatePublicApiKey(assistantId, user.getUsername()));
                            });
                })
                .map(apiKeyResult -> ResponseEntity.ok((Object) apiKeyResult))
                .onErrorResume(error -> {
                    log.warn("Failed to generate public API key for AI Assistant {}: {}", assistantId, error.getMessage());
                    ErrorResponse errorResponse = ErrorResponse.fromErrorCode(
                            ErrorCode.FORBIDDEN,
                            error.getMessage(),
                            HttpStatus.FORBIDDEN.value()
    ...
    public Mono<ResponseEntity<Object>> getWidgetScript(
        ...
        return aiAssistantService.getWidgetScriptUrl(assistantId)
                ...
                .onErrorResume(error -> {
                    log.warn("Failed to get widget script for AI Assistant {}: {}", assistantId, error.getMessage());
                    ErrorResponse errorResponse = ErrorResponse.fromErrorCode(
                            ErrorCode.INTERNAL_ERROR,
                            error.getMessage(),
                            HttpStatus.INTERNAL_SERVER_ERROR.value()
                ...
                .flatMap(existingAssistant -> {
                    return userContextService.getCurrentUsername(exchange)
                            ...
                            .flatMap(user -> {
                                // Check authorization: SUPER_ADMIN or team ADMIN
                                if (user.getRoles().contains("SUPER_ADMIN")) {
                                    return aiAssistantService.updateWidgetConfig(assistantId, widgetConfig, user.getUsername());
                                }
                                
                                return teamService.hasRole(existingAssistant.getTeamId(), user.getId(), "ADMIN")
                                        .filter(hasRole -> hasRole)
                                        .switchIfEmpty(Mono.error(new RuntimeException("Admin access required")))
                                        .flatMap(hasRole -> aiAssistantService.updateWidgetConfig(assistantId, widgetConfig, user.getUsername()));
                            });
                })
                .map(updatedAssistant -> ResponseEntity.ok((Object) updatedAssistant))
                .onErrorResume(error -> {
                    log.warn("Failed to update widget config for AI Assistant {}: {}", assistantId, error.getMessage());
                    ErrorResponse errorResponse = ErrorResponse.fromErrorCode(
                            ErrorCode.FORBIDDEN,
                            error.getMessage(),
                            HttpStatus.FORBIDDEN.value()
    ...
    public Mono<ResponseEntity<Map<String, Object>>> startConversation(
        ...
        return aiAssistantService.getAssistantById(assistantId)
                .switchIfEmpty(Mono.error(new IllegalArgumentException("AI Assistant not found with ID: " + assistantId)))
                .flatMap(assistant -> 
                    userContextService.getCurrentUsername(exchange)
                            ...
                            .flatMap(user -> teamContextService.getTeamFromContext(exchange)
                                    .switchIfEmpty(Mono.error(new IllegalArgumentException("Team not found in context")))
                                    .flatMap(teamId -> {
                                        // Check authorization
                                        if (!user.getRoles().contains("SUPER_ADMIN") && !assistant.getTeamId().equals(teamId)) {
                                            Map<String, Object> errorBody = new HashMap<>();
                                            errorBody.put("error", "Unauthorized access to assistant");
                                        ...
                                        chat.setContext((Map<String, Object>) requestBody.getOrDefault("context", new HashMap<>()));
                                        query.setChat(chat);
                                        ...
                                        return chatExecutionService.executeChat(linqRequest)
                                                .map(response -> {
                                                    ...
                                                    if (response.getChatResult() != null) {
                                                        result.put("conversationId", response.getChatResult().getConversationId());
                                                        result.put("message", response.getChatResult().getMessage());
                                                        result.put("intent", response.getChatResult().getIntent());
                                                        result.put("modelCategory", response.getChatResult().getModelCategory());
                                                        result.put("modelName", response.getChatResult().getModelName());
                                                        result.put("executedTasks", response.getChatResult().getExecutedTasks());
                                                        result.put("taskResults", response.getChatResult().getTaskResults());
                                                        result.put("tokenUsage", response.getChatResult().getTokenUsage());
                                                        result.put("metadata", response.getChatResult().getMetadata());
                                                    }
                ...
                .onErrorResume(error -> {
                    ...
                    if (errorMessage == null || errorMessage.isEmpty()) {
                        errorMessage = error.getClass().getSimpleName() + ": " + (error.getCause() != null ? error.getCause().getMessage() : "Unknown error");
                    }
