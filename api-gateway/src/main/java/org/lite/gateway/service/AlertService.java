...
public class AlertService {
    ...
    public Mono<Void> processHealthStatus(String routeId, boolean isHealthy, Map<String, Double> metrics,
        ...
        // Send notification based on severity - ONLY in Cloud/Prod profiles
        // AND only after a certain threshold to avoid deployment noise
        AlertSeverity severity = AlertSeverity.fromConsecutiveFailures(consecutiveFailures);

        if (profileService.isCloud() && consecutiveFailures >= 5) { // Increased threshold to 5 for notifications
            String subject = severity == AlertSeverity.CRITICAL ? "CRITICAL Service Alert" : "Service Health Alert";
        ...
        return Flux.fromIterable(rules)
                .flatMap(rule -> {
                    // For health alerts, we use a minimum threshold of 5 to avoid noise during
                    // deployments
                    int effectiveThreshold = "health".equals(rule.getMetric()) ? Math.max(rule.getThreshold().intValue(), 5)
                            : rule.getThreshold().intValue();

                    boolean healthAlert = consecutiveFailures >= effectiveThreshold;
                    boolean metricAlert = shouldCreateAlert(rule, metrics);
                    return Mono.just(healthAlert || metricAlert)
                            .filter(shouldAlert -> shouldAlert)
                            .flatMap(__ -> {
                                if (consecutiveFailures >= effectiveThreshold) {
                                    log.warn(
                                            "Service {} has failed {} times consecutively (threshold: {}), severity: {}",
                                            routeId, consecutiveFailures, effectiveThreshold, severity);
