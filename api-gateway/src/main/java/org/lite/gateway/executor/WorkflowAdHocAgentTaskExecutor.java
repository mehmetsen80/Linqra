...
/**
 * Executes ad-hoc WORKFLOW_EMBEDDED tasks with timeout + retry and persists
 * execution lifecycle.
 */
@Slf4j
@Component
public class WorkflowAdHocAgentTaskExecutor extends AgentTaskExecutor {
        ...
        public WorkflowAdHocAgentTaskExecutor(
                ...
                super(linqWorkflowService, workflowExecutionService, agentRepository, agentTaskRepository,
                                agentExecutionRepository, objectMapper, executionMonitoringService);
        }
        ...
        public Mono<Object> executeAdhocTask(AgentTask agentTask, String teamId, String executedBy,
                ...
                if (type != AgentTaskType.WORKFLOW_EMBEDDED && type != AgentTaskType.WORKFLOW_EMBEDDED_ADHOC) {
                        return Mono.error(new IllegalArgumentException(
                                        "Only WORKFLOW_EMBEDDED(_ADHOC) tasks are supported for ad-hoc execution"));
                }
                if (agentTask.getLinqConfig() == null) {
                        return Mono.error(new IllegalArgumentException(
                                        "Invalid task configuration: missing linq_config"));
                }

                int retries = Math.max(0, agentTask.getMaxRetries());
                Duration timeout = getTimeoutForTaskType(agentTask);

                // create execution (no persisted taskId in ad-hoc, generate a synthetic one if
                // null)
