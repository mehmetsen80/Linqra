...
public interface LinqLlmModelService {
     ...
     * 
     * @param modelCategory The model category to filter by (e.g., "openai-chat",
     *                      "gemini-chat")
     * @param teamId        The team ID to filter models
     * @return Flux of all LinqLlmModel configurations matching the category and
     *         team
     */
     ...
     * 
     * @param targets List of model categories to search (e.g., ["openai-chat",
     *                "gemini-chat"])
     * @param teamId  The team ID to filter models
     * @return Flux of all LinqLlmModel configurations matching any of the
     *         categories and team
     */
    Flux<LinqLlmModel> findByModelCategoriesAndTeamId(List<String> targets, String teamId);

    /**
     * Find a specific LinqLlmModel configuration by model category, model name, and
     * team
     * 
     ...
     * Prioritizes cheaper providers (Gemini, Cohere) first, then selects based on
     * actual pricing
     * 
     * @param modelCategories           List of model categories to search (e.g.,
     *                                  ["gemini-chat", "cohere-chat",
     *                                  "openai-chat"])
     * @param teamId                    Team ID to filter models
     * @param estimatedPromptTokens     Estimated prompt tokens for cost calculation
     *                                  (default: 1000)
     * @param estimatedCompletionTokens Estimated completion tokens for cost
     *                                  calculation (default: 500)
     * @return Mono of the cheapest available LinqLlmModel
     */
    Mono<LinqLlmModel> findCheapestAvailableModel(List<String> modelCategories, String teamId,
            long estimatedPromptTokens, long estimatedCompletionTokens);

    /**
     * Update priorities for multiple LinqLlmModel configurations
     * 
     * @param teamId          The team ID for validation
     * @param priorityUpdates Map of model ID to new priority value
     * @return Mono of void indicating completion
     */
