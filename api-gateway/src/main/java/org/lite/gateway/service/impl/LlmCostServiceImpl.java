...
public class LlmCostServiceImpl implements LlmCostService {
    ...
    private static final Duration CACHE_EXPIRATION = Duration.ofHours(24); // Cache expires in 24 hours, refreshed on
                                                                           // startup

    // In-memory cache for instant access (no network calls, no blocking)
    // This is populated on startup alongside Redis
    private volatile Map<String, Map<String, Double>> inMemoryPricingCache = new HashMap<>();
     ...
     * Pricing is cached both in-memory (for instant access) and Redis (for
     * distributed scenarios)
     */
    @EventListener(ApplicationReadyEvent.class)
    public void initializePricingCache() {
        log.info("Initializing LLM pricing cache from database...");
        ...
        llmModelRepository.findAll()
                ...
                .doOnNext(models -> {
                    ...
                    try {
                        ...
                        // Store in in-memory cache (primary - instant access, no blocking)
                        inMemoryPricingCache = cacheMap;
                        log.info("✅ Loaded {} models into in-memory pricing cache", models.size());
     ...
     */
    public Mono<Void> refreshPricingCache() {
        log.info("Refreshing LLM pricing cache from database...");

        return llmModelRepository.findAll()
                .collectList()
                .doOnNext(models -> {
                    if (models == null || models.isEmpty()) {
                        log.warn("⚠️ No models found in database during cache refresh");
                        return;
                    }
                    ...
                    try {
                        ...
                        inMemoryPricingCache = cacheMap;
                        log.info("✅ Refreshed {} models into in-memory pricing cache", models.size());
    ...
    public Mono<LlmUsageStats> getTeamLlmUsage(String teamId, String fromDate, String toDate) {
        ...
        Mono<java.util.List<LinqWorkflowExecution>> executionsMono = executionRepository
                .findByTeamIdAndExecutedAtBetween(teamId, from, to)
                .collectList();

        // Get conversations for the team, filter by date range, then get all messages
        Mono<java.util.List<ConversationMessage>> chatMessagesMono = conversationRepository
                .findByTeamIdOrderByStartedAtDesc(teamId)
                .filter(conversation -> {
                    LocalDateTime startedAt = conversation.getStartedAt() != null ? conversation.getStartedAt()
                            : conversation.getCreatedAt();
                    return startedAt != null && !startedAt.isBefore(from) && !startedAt.isAfter(to);
                })
                .flatMap(conversation -> conversationMessageRepository
                        .findByConversationIdOrderBySequenceNumberAsc(conversation.getId()))
                .filter(message -> {
                    // Filter messages by timestamp/createdAt within date range
                    LocalDateTime messageTime = message.getTimestamp() != null ? message.getTimestamp()
        ...
        Mono<java.util.List<KnowledgeHubDocumentMetaData>> metadataMono = metadataRepository.findAll()
                ...
                .filter(metadata -> {
                    LocalDateTime extractedAt = metadata.getExtractedAt() != null ? metadata.getExtractedAt()
    ...
    private String normalizeModelName(String model) {
        ...
        // Remove short numeric version suffixes (3 digits or more)
        // Examples: gpt-4-0613, gpt-3.5-turbo-0125, gemini-1.5-pro-001
        // Do NOT remove single digits like in claude-sonnet-4-5
        model = model.replaceAll("-\\d{3,}$", "");
    ...
    private double calculateCostFromCache(String model, long promptTokens, long completionTokens) {
        // First, try in-memory cache (instant access, no network, no blocking)
        if (!inMemoryPricingCache.isEmpty()) {
            Map<String, Double> pricing = inMemoryPricingCache.get(model);
            if (pricing != null) {
            ...
            // Model not found in cache - log error and return 0
            log.error("❌ Model {} not found in pricing cache. Please add this model to the database.", model);
            return 0.0;
        }
        ...
        try {
            ...
            if (cacheJson != null && !cacheJson.isEmpty()) {
                ...
                inMemoryPricingCache = cacheMap;
                log.info("✅ Populated in-memory cache from Redis with {} models", cacheMap.size());
                ...
                // Model not found in Redis cache either
                log.error("❌ Model {} not found in pricing cache. Please add this model to the database.", model);
                return 0.0;
            }
        } catch (Exception e) {
            log.error("Error reading Redis pricing cache for model {}: {}", model, e.getMessage());
        }

        // Both caches empty - log error and return 0
        log.error("❌ Pricing cache is empty. Ensure application startup completed and database has models.");
        return 0.0;
    }
    ...
    private LlmUsageStats calculateUsageStats(String teamId,
        ...
        for (LinqWorkflowExecution execution : executions) {
            ...
            for (LinqResponse.WorkflowStepMetadata stepMetadata : execution.getResponse().getMetadata()
                    .getWorkflowMetadata()) {
                ...
                if (modelCategory.startsWith("ollama")) {
                    ...
                    log.debug("Calculated cost for {} ({}) with current pricing: ${}", model, rawModel,
                ...
                double newAvg = (currentAvg * (modelUsage.getRequests() - 1) + stepMetadata.getDurationMs())
                        / modelUsage.getRequests();
        ...
        for (ConversationMessage message : chatMessages) {
            if (!"ASSISTANT".equals(message.getRole()) || message.getMetadata() == null
                    || message.getMetadata().getTokenUsage() == null) {
                continue;
            }
        ...
        // 3. Process knowledge hub graph extraction costs (entity and relationship
        // extraction)
        for (KnowledgeHubDocumentMetaData metadata : metadataList) {
            ...
            if (graphExtraction.containsKey("entityExtraction")) {
                ...
                if (entityExtraction.containsKey("costUsd") && entityExtraction.get("costUsd") instanceof Number) {
                    long promptTokens = entityExtraction.containsKey("promptTokens")
                            ? ((Number) entityExtraction.get("promptTokens")).longValue()
                            : 0;
                    long completionTokens = entityExtraction.containsKey("completionTokens")
                            ? ((Number) entityExtraction.get("completionTokens")).longValue()
                            : 0;
                    ...
                    if (cost > 0) {
                        // Use a default model name for graph extraction (could be improved to store
                        // actual model used)
            ...
            if (graphExtraction.containsKey("relationshipExtraction")) {
                ...
                if (relationshipExtraction.containsKey("costUsd")
                        && relationshipExtraction.get("costUsd") instanceof Number) {
                    long promptTokens = relationshipExtraction.containsKey("promptTokens")
                            ? ((Number) relationshipExtraction.get("promptTokens")).longValue()
                            : 0;
                    long completionTokens = relationshipExtraction.containsKey("completionTokens")
                            ? ((Number) relationshipExtraction.get("completionTokens")).longValue()
                            : 0;
        ...
        for (LinqWorkflowExecution execution : executions) {
            ...
            for (LinqResponse.WorkflowStepMetadata stepMetadata : execution.getResponse().getMetadata()
                ...
                // Get the execution date (use executedAt from step metadata if available,
                // otherwise from execution)
        ...
        for (KnowledgeHubDocumentMetaData metadata : metadataList) {
            ...
            if (graphExtraction.containsKey("entityExtraction")) {
                ...
                if (entityExtraction.containsKey("costUsd") && entityExtraction.get("costUsd") instanceof Number) {
                    long tokens = entityExtraction.containsKey("totalTokens")
                            ? ((Number) entityExtraction.get("totalTokens")).longValue()
                            : ((entityExtraction.containsKey("promptTokens")
                                    ? ((Number) entityExtraction.get("promptTokens")).longValue()
                                    : 0) +
                                    (entityExtraction.containsKey("completionTokens")
                                            ? ((Number) entityExtraction.get("completionTokens")).longValue()
                                            : 0));
                    double cost = ((Number) entityExtraction.get("costUsd")).doubleValue();
            ...
            if (graphExtraction.containsKey("relationshipExtraction")) {
                ...
                if (relationshipExtraction.containsKey("costUsd")
                    ...
                    long tokens = relationshipExtraction.containsKey("totalTokens")
                            ? ((Number) relationshipExtraction.get("totalTokens")).longValue()
                            : ((relationshipExtraction.containsKey("promptTokens")
                                    ? ((Number) relationshipExtraction.get("promptTokens")).longValue()
                                    : 0) +
                                    (relationshipExtraction.containsKey("completionTokens")
                                            ? ((Number) relationshipExtraction.get("completionTokens")).longValue()
                                            : 0));
                    double cost = ((Number) relationshipExtraction.get("costUsd")).doubleValue();
        ...
        log.info(
                "Calculated LLM usage for team {}: {} requests, ${} total cost, {} days (workflows: {}, chat: {}, graph: {})",
    ...
    @Override
    public Mono<Double> calculateCostForMonth(String teamId, String model, long promptTokens, long completionTokens,
            YearMonth yearMonth) {
        return pricingSnapshotRepository.findByTeamIdAndYearMonthAndModel(teamId, yearMonth.toString(), model)
                .switchIfEmpty(pricingSnapshotRepository.findByYearMonthAndModel(yearMonth.toString(), model)) // Fallback
                                                                                                               // to
                                                                                                               // global
                                                                                                               // pricing
    ...
    public Flux<LlmPricingSnapshot> getPricingSnapshotsForMonth(String teamId, YearMonth yearMonth) {
        return pricingSnapshotRepository.findByTeamIdAndYearMonth(teamId, yearMonth.toString())
                .switchIfEmpty(pricingSnapshotRepository.findByYearMonth(yearMonth.toString())); // Fallback to global
                                                                                                 // pricing
    }
    ...
    public Mono<Void> backfillHistoricalPricing(String teamId, YearMonth fromYearMonth, YearMonth toYearMonth) {
        ...
        return cleanupAllPricingDuplicates()
                ...
                .doOnSuccess(v -> log.info("Historical pricing backfill complete for team {} from {} to {}", teamId,
     ...
     */
    private Mono<LlmPricingSnapshot> cleanupDuplicatesAndSave(String yearMonth, String modelName,
            LlmPricingSnapshot templateSnapshot) {
    ...
    private Mono<Void> initializePricingForMonth(String teamId, YearMonth yearMonth) {
        ...
        return pricingSnapshotRepository.findByTeamIdAndYearMonth(teamId, yearMonth.toString())
                ...
                .flatMap(hasSnapshots -> {
                    if (hasSnapshots) {
                        log.info("Pricing snapshots already exist for team {} in {}, skipping initialization", teamId,
                                yearMonth);
                        return Mono.empty();
                    }
    ...
    private Mono<Void> initializePricingForMonthInternal(String teamId, YearMonth yearMonth) {
        ...
        return llmModelRepository.findByActive(true)
                ...
                .concatMap(model -> {
                    ...
                    // Check if already exists for this team (team-specific check with fallback to
                    // global)
                    return pricingSnapshotRepository
                            .findByTeamIdAndYearMonthAndModel(teamId, yearMonth.toString(), modelName)
                            .flatMap(existing -> {
                                log.debug("Pricing snapshot already exists for team {} / {} in {}, skipping", teamId,
                                        modelName, yearMonth);
                                return Mono.empty();
                            })
                            .switchIfEmpty(
                                    pricingSnapshotRepository.save(snapshot)
                                            .doOnSuccess(saved -> log.info(
                                                    "Saved pricing snapshot for team {} / {} in {}: ${}/{}",
                                                    teamId, modelName, yearMonth, model.getInputPricePer1M(),
                                            ...
                                            .onErrorResume(error -> {
                                                ...
                                                if (error.getMessage() != null
                                                    ...
                                                    log.debug(
                                                            "Pricing snapshot already exists (duplicate key), skipping");
                                                    return Mono.empty();
                                                }
    ...
    public Mono<Integer> backfillExecutionCosts(String teamId, boolean dryRun) {
        log.info(
                "🔄 Starting cost backfill for executions, chat conversations, and graph extraction (dryRun: {}, teamId: {})",
                dryRun, teamId);
        ...
        return Mono.zip(executionsCount, chatCount, graphCount)
                .map(tuple -> {
                    ...
                    if (dryRun) {
                        log.info(
                                "✅ [DRY RUN] Would update {} executions, {} chat messages, {} graph extractions (total: {})",
                                execCount, chatCountResult, graphCountResult, total);
                    } else {
                        log.info(
                                "✅ Successfully backfilled costs: {} executions, {} chat messages, {} graph extractions (total: {})",
                                execCount, chatCountResult, graphCountResult, total);
                    }
                    return total;
                })
        ...
        return executionsFlux
                .filter(execution -> {
                    ...
                    return execution.getResponse().getMetadata().getWorkflowMetadata().stream()
                            .anyMatch(stepMetadata -> {
                                String modelCategory = stepMetadata.getTarget();
                                boolean isLlmStep = modelCategory.equals("openai-chat")
                                        || modelCategory.equals("gemini-chat")
                                        || modelCategory.equals("cohere-chat") || modelCategory.equals("claude-chat")
                                        || modelCategory.equals("openai-embed")
                ...
                .flatMap(execution -> {
                    ...
                    for (LinqResponse.WorkflowStepMetadata stepMetadata : execution.getResponse().getMetadata()
                        ...
                        if (!modelCategory.equals("openai-chat") && !modelCategory.equals("gemini-chat")
                                && !modelCategory.equals("cohere-chat")
                                && !modelCategory.equals("claude-chat") && !modelCategory.equals("openai-embed")
                                && !modelCategory.equals("gemini-embed") && !modelCategory.equals("cohere-embed")) {
                            continue;
                        }
                        ...
                        boolean needsModelUpdate = (stepMetadata.getModel() == null
                                || stepMetadata.getModel().isEmpty());
                        ...
                        long promptTokens = tokenUsage.getPromptTokens();
                        long completionTokens = tokenUsage.getCompletionTokens();
                        double cost = needsCostUpdate ? calculateCost(model, promptTokens, completionTokens)
                                : tokenUsage.getCostUsd();
                        ...
                        log.info("📊 {} execution {} - step {} ({}): model={}, tokens={}p/{}c, cost=${}, updates=[{}]",
                                ...
                                completionTokens,
                                String.format("%.6f", cost),
                                (needsCostUpdate ? "cost" : "") + (needsCostUpdate && needsModelUpdate ? "," : "")
                                        + (needsModelUpdate ? "model" : ""));
        ...
        return messagesFlux
                ...
                .flatMap(message -> {
                    ...
                    String model = modelName != null && !modelName.isEmpty() ? modelName
                            : getDefaultModelForCategory(modelCategory);

                    long promptTokens = tokenUsage.getPromptTokens() != null ? tokenUsage.getPromptTokens() : 0;
                    long completionTokens = tokenUsage.getCompletionTokens() != null ? tokenUsage.getCompletionTokens()
                            : 0;
    ...
    /**
     * Backfill costs for knowledge graph extraction (entity and relationship
     * extraction)
     */
    private Mono<Integer> backfillGraphExtractionCosts(String teamId, boolean dryRun) {
        ...
        return metadataFlux
                .filter(metadata -> {
                    ...
                    if (metadata.getCustomMetadata() == null
                            || !metadata.getCustomMetadata().containsKey("graphExtraction")) {
                        return false;
                    }
                    ...
                    // Check if entityExtraction or relationshipExtraction has token usage but
                    // missing cost
                    ...
                    if (graphExtraction.containsKey("entityExtraction")) {
                        ...
                        if (entityExtraction.containsKey("promptTokens")
                                || entityExtraction.containsKey("completionTokens")) {
                            Object costObj = entityExtraction.get("costUsd");
                            entityNeedsCost = costObj == null
                                    || (costObj instanceof Number && ((Number) costObj).doubleValue() == 0.0);
                        }
                    }
                    ...
                    if (graphExtraction.containsKey("relationshipExtraction")) {
                        ...
                        Map<String, Object> relationshipExtraction = (Map<String, Object>) graphExtraction
                                .get("relationshipExtraction");
                        if (relationshipExtraction.containsKey("promptTokens")
                                || relationshipExtraction.containsKey("completionTokens")) {
                            Object costObj = relationshipExtraction.get("costUsd");
                            relationshipNeedsCost = costObj == null
                                    || (costObj instanceof Number && ((Number) costObj).doubleValue() == 0.0);
                        }
                    }
                ...
                .flatMap(metadata -> {
                    ...
                    if (graphExtraction.containsKey("entityExtraction")) {
                        ...
                        Object costObj = entityExtraction.get("costUsd");
                        boolean needsCost = costObj == null
                                || (costObj instanceof Number && ((Number) costObj).doubleValue() == 0.0);
                        ...
                        if (needsCost) {
                            ...
                            if (promptTokens > 0 || completionTokens > 0) {
                                ...
                                log.info(
                                        "🔗 {} graph entity extraction (document {}): model={}, tokens={}p/{}c, cost=${}",
                                        dryRun ? "[DRY RUN]" : "Updating",
                    ...
                    if (graphExtraction.containsKey("relationshipExtraction")) {
                        ...
                        if (needsCost) {
                            ...
                            if (promptTokens > 0 || completionTokens > 0) {
                                ...
                                log.info(
                                        "🔗 {} graph relationship extraction (document {}): model={}, tokens={}p/{}c, cost=${}",
                                        dryRun ? "[DRY RUN]" : "Updating",
                                        metadata.getDocumentId(),
                                        defaultModel,
                                        promptTokens,
    ...
    public Mono<Integer> backfillTokenUsageFromResponses(String teamId, boolean dryRun) {
        log.info("🔄 Starting ADVANCED token usage backfill from response data (dryRun: {}, teamId: {})", dryRun,
                teamId);
        ...
        return executionsFlux
                ...
                .flatMap(execution -> {
                    boolean updated = false;
                    LinqResponse.WorkflowResult workflowResult = (LinqResponse.WorkflowResult) execution.getResponse()
                            .getResult();
                    ...
                    for (LinqResponse.WorkflowStepMetadata stepMetadata : execution.getResponse().getMetadata()
                        ...
                        if (modelCategory.equals("cohere-chat") || modelCategory.equals("cohere-embed")) {
                            ...
                            if (resultMap.containsKey("usageMetadata")) {
                                @SuppressWarnings("unchecked")
                                Map<String, Object> usageMetadata = (Map<String, Object>) resultMap
                                        .get("usageMetadata");
                        ...
                        log.info(
                                "🔍 {} execution {} - step {} ({}): EXTRACTING tokenUsage from response → model={}, tokens={}p/{}c, cost=${}",
                                dryRun ? "[DRY RUN]" : "Updating",
                                execution.getId(),
                                stepMetadata.getStep(),
                                modelCategory,
