...
public class VaultFileServiceImpl implements VaultFileService {
    ...
    // private String vaultFilePath; // Removed, passed as argument

    private VaultFile cachedVault;
    private LocalDateTime lastLoaded;
    private java.nio.file.Path lastLoadedPath;
    private final java.util.concurrent.locks.ReadWriteLock fileLock = new java.util.concurrent.locks.ReentrantReadWriteLock();

    @Override
    public VaultFile loadVault(java.nio.file.Path vaultFile) {
        fileLock.readLock().lock();
        try {
            // Return cached if still valid AND path hasn't changed
            if (cachedVault != null && lastLoaded != null && lastLoadedPath != null
                    && lastLoadedPath.equals(vaultFile)
            ...
            try {
                ...
                // Load from disk
                return loadVaultFromDisk(vaultFile);
     ...
     */
    private VaultFile loadVaultFromDisk(java.nio.file.Path vaultFilePath) {
        try {
            File vaultFile = vaultFilePath.toFile();
