...
public class KnowledgeHubCollectionServiceImpl implements KnowledgeHubCollectionService {
                            ...
                            .flatMap(hasDuplicate -> {
                                if (hasDuplicate) {
                                    return Mono.error(new RuntimeException(
                                            "Collection with name '" + name + "' already exists for this team"));
                                }
                                        ...
                                        .flatMap(savedCollection -> {
                                            ...
                                            auditContext.put("categories",
                                                    savedCollection.getCategories() != null
                                                            ? savedCollection.getCategories().stream().map(Enum::name)
                            ...
                            .flatMap(hasDuplicate -> {
                                ...
                                return collectionRepository.save(collection)
                                        .flatMap(savedCollection -> {
                                            ...
                                            auditContext.put("oldCategories",
                                                    oldCategories != null
                                                            ? oldCategories.stream().map(Enum::name)
                                                                    .collect(Collectors.toList())
                                                            : java.util.Collections.emptyList());
                                            auditContext.put("newCategories",
                                                    categories != null
                                                            ? categories.stream().map(Enum::name)
                                                                    .collect(Collectors.toList())
                                                            : java.util.Collections.emptyList());
                ...
                .flatMap(collection -> {
                    ...
                    return documentRepository.findByCollectionId(id)
                            ...
                            .flatMap(documentCount -> {
                                if (documentCount > 0) {
                                    return Mono.error(new RuntimeException(
                                            "Cannot delete collection: Collection contains " + documentCount
                                        ...
                                        .flatMap(v -> {
                                            ...
                                            auditContext.put("categories",
                                                    collectionCategories != null
                                                            ? collectionCategories.stream().map(Enum::name)
                                                                    .collect(Collectors.toList())
                                                            : java.util.Collections.emptyList());
                    ...
                    return requireMilvusCollectionForTeam(request.getMilvusCollectionName(), teamId)
                            .flatMap(collectionInfo -> {
                                ...
                                if (collectionType != null && !collectionType.equalsIgnoreCase("KNOWLEDGE_HUB")) {
                                    return Mono.error(new RuntimeException(
                                            "Milvus collection '" + request.getMilvusCollectionName()
                                                    + "' is not registered as a Knowledge Hub collection."));
                                }

                                if (milvusDimension != null && requestedDimension != null
                                        && !Objects.equals(milvusDimension, requestedDimension)) {
                                    return Mono.error(new RuntimeException(
                                            "Milvus collection '" + request.getMilvusCollectionName()
                                                    + "' has vector dimension " + milvusDimension +
                                                    " but embedding model requires " + requestedDimension));
                                }

                                Integer effectiveDimension = milvusDimension != null ? milvusDimension
                                        : requestedDimension;
                                ...
                                return validateMilvusCollectionSchema(request.getMilvusCollectionName(), teamId,
                                        effectiveDimension)
    ...
    @Override
    public Mono<Map<String, Object>> searchCollection(String collectionId, String teamId, String query, Integer topK,
        ...
        return collectionRepository.findById(collectionId)
                ...
                .flatMap(collection -> {
                    if (!Objects.equals(collection.getTeamId(), teamId)) {
                        return Mono
                                .error(new RuntimeException("Collection not found or access denied: " + collectionId));
                    }
                    ...
                    return milvusStoreService.searchRecord(
                            ...
                            .flatMap(searchResults -> {
                                ...
                                auditContext.put("metadataFilters",
                                        metadataFilters != null ? metadataFilters : java.util.Collections.emptyMap());
                                auditContext.put("resultCount",
                                        searchResults != null && searchResults instanceof Map
                                                ? ((Map<?, ?>) searchResults).size()
                                                : 0);
                            ...
                            .onErrorResume(error -> {
                                log.error("Semantic search failed for collection {}: {}", collectionId,
                                ...
                                return auditLogHelper.logDetailedEvent(
                                        ...
                                        collectionId)
                                        .doOnError(
                                                auditError -> log.error("Failed to log audit event (search failed): {}",
                                                        auditError.getMessage(), auditError))
    ...
    private Mono<MilvusCollectionInfo> requireMilvusCollectionForTeam(String collectionName, String teamId) {
        return milvusStoreService.listCollections(teamId, null)
                ...
                .switchIfEmpty(Mono.error(new RuntimeException(
                        "Milvus collection '" + collectionName + "' not found for team " + teamId)));
    }
        ...
        return milvusStoreService.getCollectionDetails()
                .flatMap(details -> {
                    ...
                    if (maybeCollection.isEmpty()) {
                        return Mono.error(new RuntimeException(
                                "Milvus collection '" + collectionName + "' not found in collection details"));
                    }
                    ...
                    if (!(schemaObj instanceof List<?> schemaList)) {
                        return Mono.error(new RuntimeException(
                                "Unable to read schema for Milvus collection '" + collectionName + "'"));
                    }
                    ...
                    if (!validationResult.isValid()) {
                        ...
                        return Mono.error(new RuntimeException(
                                "Milvus collection '" + collectionName + "' schema validation failed: " + message));
                    }
