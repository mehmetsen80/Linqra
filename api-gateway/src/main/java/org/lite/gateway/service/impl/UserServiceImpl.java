...
import org.lite.gateway.service.NotificationService; // Add import
        ...
        return Mono.just(request.getUsername())
                ...
                .switchIfEmpty(Mono.defer(() -> {
                    ...
                    return auditService.logEvent(
                            ...
                            .doOnError(auditError -> log.error("Failed to log failed login audit event: {}",
                            ...
                            .then(Mono.error(new ResourceNotFoundException(
                                    "username '" + request.getUsername() + "' does not exist",
                ...
                .flatMap(user -> {
                    if (passwordEncoder.matches(request.getPassword(), user.getPassword())) {
                        ...
                        return auditService.logEvent(
                                ...
                                .doOnError(auditError -> log.error("Failed to log login audit event: {}",
                                        auditError.getMessage(), auditError))
                                .onErrorResume(auditError -> Mono.empty()) // Don't fail if audit logging fails
                ...
                .flatMap(exists -> {
                    if (exists) {
                        ...
                        AuditLog.AuditMetadata duplicateUsernameMetadata = AuditLog.AuditMetadata.builder()
                                .reason(String.format("Registration failed - duplicate username: %s",
                                        request.getUsername()))
                        ...
                        return auditService.logEvent(
                                ...
                                .doOnError(auditError -> log.error("Failed to log failed registration audit event: {}",
                                        auditError.getMessage(), auditError))
                                .onErrorResume(auditError -> Mono.empty()) // Don't fail if audit logging fails
                ...
                .flatMap(emailExists -> {
                    if (emailExists) {
                        ...
                        AuditLog.AuditMetadata duplicateEmailMetadata = AuditLog.AuditMetadata.builder()
                                .reason(String.format("Registration failed - duplicate email for username: %s",
                                        request.getUsername()))
                    ...
                    return userRepository.save(newUser)
                            .flatMap(savedUser -> {
                                ...
                                AuditLog.AuditMetadata registrationMetadata = AuditLog.AuditMetadata.builder()
                                        .reason(String.format("User registration successful for username: %s",
                                ...
                                return auditService.logEvent(
                                        ...
                                        .doOnError(auditError -> log.error("Failed to log registration audit event: {}",
                                                auditError.getMessage(), auditError))
                                        .onErrorResume(auditError -> Mono.empty()) // Don't fail if audit logging fails
                                        .then(notificationService
                                                .sendWelcomeEmail(savedUser.getEmail(), savedUser.getUsername())
                                                .doOnError(e -> log.error("Failed to send welcome email to user {}: {}",
