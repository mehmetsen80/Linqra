      ...
      if (defaultModel) {
        setFormData(prev => ({
          ...
          defaultModel: {
            ...
            modelCategory: defaultModel.modelCategory
          }
        }));
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        showErrorToast(response.error || 'Failed to create AI assistant');
      }
    } catch (error) {
      console.error('Error creating AI assistant:', error);
      showErrorToast(error.response?.data?.message || 'Failed to create AI assistant');
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <Modal show={show} onHide={onHide} centered size="lg">
      <Modal.Header closeButton>
        <Modal.Title>Create New AI Assistant</Modal.Title>
      </Modal.Header>
      <Form onSubmit={handleSubmit}>
                  ...
                  <Col md={6}>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Select
                        ...
                        value={formData.defaultModel.provider}
                  ...
                  <Col md={6}>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control
                        ...
                        value={formData.contextManagement.maxRecentMessages}
                        onChange={handleInputChange}
                      />
                      <Form.Text className="text-muted">
                        Number of most recent messages from the conversation to include in each API request. If set to 10, only the last 10 messages (user + assistant pairs) will be sent to the LLM, even if the conversation has 100+ messages stored in the database. <br /> <br /> However, if these messages exceed the "Max Total Tokens" limit, fewer messages will be included to respect the token budget.
                      </Form.Text>
                    </Form.Group>
                  </Col>
                  <Col md={6}>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control
                        ...
                        value={formData.contextManagement.maxTotalTokens}
                        onChange={handleInputChange}
                      />
                      <Form.Text className="text-muted">
                        Maximum tokens from conversation history to include in each API request. Full conversation is stored in database, but only this amount will be sent to the LLM per request to manage context window and costs. <br /> <br /> This works together with "Max Recent Messages" - if the recent messages exceed this token limit, fewer messages will be sent to stay within the budget, prioritizing the most recent messages first.
                      </Form.Text>
                    </Form.Group>
                  </Col>
                </Row>
              ...
              <Accordion.Body>
                <Form.Group className="mb-3">
                  <Form.Check
                    ...
                    checked={formData.guardrails.piiDetectionEnabled}
                    onChange={handleInputChange}
                  />
                  <Form.Text className="text-muted">
                ...
                <Form.Group className="mb-3">
                  <Form.Check
                    ...
                    checked={formData.guardrails.auditLoggingEnabled}
                    onChange={handleInputChange}
                  />
                  <Form.Text className="text-muted">
        ...
        <Modal.Footer>
          ...
          <Button
            variant="primary"
            type="submit"
            disabled={creating || !formData.name || !formData.description || !formData.defaultModel.modelName}
          >
            {creating ? 'Creating...' : 'Create AI Assistant'}
          </Button>
        </Modal.Footer>
      </Form>
    </Modal>
