...
public class AuditController {
    ...
    @Operation(summary = "Query audit logs", 
               description = "Query audit logs with filters (team, user, event type, time range, etc.) and pagination. " +
                           "Only ADMIN or SUPER_ADMIN can query audit logs.")
                ...
                .flatMap(user -> teamContextService.getTeamFromContext(exchange)
                        .flatMap(teamId -> {
                            ...
                            if (user.getRoles().contains("SUPER_ADMIN")) {
                                log.debug("User {} authorized as SUPER_ADMIN to query audit logs", user.getUsername());
                            ...
                            return teamContextService.getTeamFromContext(exchange)
                                    .flatMap(actualTeamId -> {
                                        ...
                                        if (request.getTeamId() != null && !request.getTeamId().equals(actualTeamId)) {
                                        ...
                                        // Set teamId from context if not provided
                                        if (request.getTeamId() == null) {
                                            request.setTeamId(actualTeamId);
                                        }
                                        
                                        return executeQuery(request, actualTeamId);
                                    });
                        }))
                .onErrorResume(error -> {
                    ...
                    return Mono.just(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                            .body(ErrorResponse.fromErrorCode(
                                    ...
                                    "Error querying audit logs: " + error.getMessage(),
    ...
    @Operation(summary = "Get document audit trail", 
               description = "Get complete audit trail for a specific document. Only ADMIN or SUPER_ADMIN can access.")
        ...
        return userContextService.getCurrentUsername(exchange)
                ...
                .onErrorResume(error -> {
                    ...
                    return Mono.just(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                            .body(ErrorResponse.fromErrorCode(
                                    org.lite.gateway.dto.ErrorCode.INTERNAL_ERROR,
                                    "Error getting document audit trail: " + error.getMessage(),
                                    HttpStatus.INTERNAL_SERVER_ERROR.value())));
                });
    }
    ...
    @Operation(summary = "Get user audit logs", 
               description = "Get audit logs for a specific user within a time range. Only ADMIN or SUPER_ADMIN can access.")
    public Mono<ResponseEntity<?>> getUserAuditLogs(
        ...
        LocalDateTime start = startTime != null ? 
                LocalDateTime.parse(startTime) : LocalDateTime.now().minusDays(90);
        ...
        return userContextService.getCurrentUsername(exchange)
                ...
                .onErrorResume(error -> {
                    ...
                    return Mono.just(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                            .body(ErrorResponse.fromErrorCode(
                                    org.lite.gateway.dto.ErrorCode.INTERNAL_ERROR,
                                    "Error getting user audit logs: " + error.getMessage(),
                                    HttpStatus.INTERNAL_SERVER_ERROR.value())));
                });
    }
    
    private Mono<ResponseEntity<?>> getUserLogs(String userId, String teamId, LocalDateTime start, LocalDateTime end) {
    ...
    @Operation(summary = "Get archival statistics", 
               description = "Get statistics about audit log archival status. Only ADMIN or SUPER_ADMIN can access.")
                ...
                .flatMap(user -> {
                    ...
                    if (user.getRoles().contains("SUPER_ADMIN")) {
                        return archivalService.getArchivalStats()
                                .map(stats -> (ResponseEntity<?>) ResponseEntity.ok(stats));
                    }
                    
                    // For team ADMIN, they can see stats for their team
                    return archivalService.getArchivalStats()
                            .map(stats -> (ResponseEntity<?>) ResponseEntity.ok(stats));
                })
                .onErrorResume(error -> {
                    ...
                    return Mono.just(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                            .body(ErrorResponse.fromErrorCode(
                                    org.lite.gateway.dto.ErrorCode.INTERNAL_ERROR,
                                    "Error getting archival stats: " + error.getMessage(),
                                    HttpStatus.INTERNAL_SERVER_ERROR.value())));
                });
    }
}
