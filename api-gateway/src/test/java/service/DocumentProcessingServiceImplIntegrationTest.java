...
class DocumentProcessingServiceImplIntegrationTest {
    ...
    void testProcessDocument_CompleteFlow() {
        ...
        assertEquals(DocumentStatus.UPLOADED, beforeProcessing.getStatus());
        ...
        assertEquals(DocumentStatus.PROCESSED, afterProcessing.getStatus());
        ...
        StepVerifier.create(chunks)
                .thenConsumeWhile(chunk -> {
                    ...
                    // Verify encryption key version is set (even if using no-op encryption for
                    // tests)
    ...
    void testProcessDocument_ExistingUSCISDocument() {
        // Test processing with an existing USCIS form document that's already uploaded
        // to S3
        if ("YOUR_AWS_ACCESS_KEY".equals(AWS_ACCESS_KEY) ||
                "YOUR_AWS_SECRET_KEY".equals(AWS_SECRET_KEY)) {
            System.out.println("⚠️  Skipping integration test - AWS credentials not set");
        ...
        if (existingDocument == null) {
            System.out.println(
                    "❌ Document not found in MongoDB. Please check the documentId: " + EXISTING_USCIS_DOCUMENT_ID);
        ...
        if (existingDocument.getStatus() != DocumentStatus.UPLOADED
                && existingDocument.getStatus() != DocumentStatus.PARSING) {
            System.out.println("⚠️  Document status is: " + existingDocument.getStatus()
                    + ". Updating to UPLOADED for processing...");
    ...
    void testProcessDocument_ErrorHandling() {
        ...
        String invalidS3Key = "raw/" + TEST_TEAM_ID + "/" + TEST_COLLECTION_ID + "/nonexistent/" + documentId + "_"
                + fileName;
