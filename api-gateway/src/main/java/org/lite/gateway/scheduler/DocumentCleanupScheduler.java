                ...
                documentRepository.findByStatusAndDeletedAtBefore(DocumentStatus.DELETED, threshold)
                                .flatMap(doc -> {
                                        ...
                                        return documentService.hardDeleteDocument(doc.getDocumentId(), doc.getTeamId())
                                                        ...
                                                        .onErrorResume(e -> {
                                                                log.error("Failed to cleanup document {}: {}",
                                ...
                                .subscribe(
                                                count -> log.info(
                                                                "Trash Can cleanup completed. Permanently deleted {} documents.",
                                                                count),
                                                error -> log.error("Trash Can cleanup failed: {}", error.getMessage(),
                                                                error));
        }
}
