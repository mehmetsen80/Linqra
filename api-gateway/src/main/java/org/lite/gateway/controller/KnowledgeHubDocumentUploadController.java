...
public class KnowledgeHubDocumentUploadController {
                ...
                return teamContextService.getTeamFromContext(exchange)
                                .flatMap(teamId -> documentService.initiateDocumentUpload(request, teamId)
                                                .map(result -> {
                                                        ...
                                                        return ResponseEntity.ok(UploadResponse.builder()
                                                                        ...
                                                                        .expiresInSeconds(presignedUrl
                                                                                        .getExpiresInSeconds())
                                                                        .requiredHeaders(presignedUrl
                                                                                        .getRequiredHeaders())
                                ...
                                .onErrorResume(error -> {
                                        ...
                                        return Mono.just(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                                        .body(UploadResponse.error("Failed to initiate upload: "
                                                                        + error.getMessage())));
                                });
        }
                ...
                return teamContextService.getTeamFromContext(exchange)
                                .flatMap(teamId -> {
                                        ...
                                        for (UploadInitiateRequest fileRequest : request.getFiles()) {
                                                ...
                                                uploadResponses.add(documentService
                                                                .initiateDocumentUpload(fileRequest, teamId)
                                                                .map(result -> {
                                                                        var presignedUrl = result.presignedUrl();
                                                                        var document = result.document();
                                                        ...
                                                        .map(uploads -> {
                                                                ...
                                                                return ResponseEntity.ok(BulkUploadResponse.builder()
                                                                                ...
                                                                                .message(String.format(
                                                                                                "Initiated %d/%d files successfully",
                                                                                                successCount,
                                                                                                uploads.size()))
                                                                                .build());
                                                        });
                                ...
                                .onErrorResume(error -> {
                                        ...
                                        return Mono.just(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                                        .body(BulkUploadResponse
                                                                        .error("Failed to initiate bulk upload: "
                                                                                        + error.getMessage())));
                                });
        }
        ...
        @AuditLog(eventType = AuditEventType.DOCUMENT_UPLOADED, action = AuditActionType.CREATE, resourceType = AuditResourceType.DOCUMENT, resourceIdParam = "documentId", documentIdParam = "documentId", reason = "Document upload completed and processing queued")
        public Mono<ResponseEntity<UploadResponse>> completeUpload(
                ...
                // Note: We skip fileExists check because presigned URLs are already
                // authenticated
                ...
                return teamContextService.getTeamFromContext(exchange)
                                .flatMap(teamId -> documentService.completeUpload(documentId, request.getS3Key())
                                                .map(document -> ResponseEntity.ok(UploadResponse.builder()
                                                                ...
                                                                .build()))
                                                .onErrorResume(error -> {
                                                        log.error("Error completing upload for document: {}",
                                                                        documentId, error);
                                                        // Mark document as FAILED with error message
                                                        return documentService
                                                                        .updateStatus(documentId, "FAILED",
                                                                                        error.getMessage())
                                                                        .then(Mono.just(ResponseEntity.status(
                                                                                        HttpStatus.INTERNAL_SERVER_ERROR)
                                                                                        .body(UploadResponse.error(
        ...
        public Mono<ResponseEntity<KnowledgeHubDocument>> getDocumentStatus(@PathVariable String documentId,
                        ServerWebExchange exchange) {
                                ...
                                .flatMap(teamId -> {
                                        ...
                                        if (collectionId != null && status != null) {
                                                ...
                                                documents = documentRepository
                                                                .findByTeamIdAndCollectionId(teamId, collectionId)
                                                                .filter(doc -> status.equals(doc.getStatus()));
                                        } else if (collectionId != null) {
                                                // Filter by collection only
                                                documents = documentRepository.findByTeamIdAndCollectionId(teamId,
                                                                collectionId);
                                        } else if (status != null) {
        ...
        public Mono<ResponseEntity<Object>> generateDownloadUrl(@PathVariable String documentId,
                        ServerWebExchange exchange) {
         ...
         * Note: This must come before the regular delete endpoint to ensure proper
         * route matching
         */
        @DeleteMapping("/{documentId}/hard")
        @AuditLog(eventType = AuditEventType.DOCUMENT_HARD_DELETED, action = AuditActionType.DELETE, resourceType = AuditResourceType.DOCUMENT, resourceIdParam = "documentId", documentIdParam = "documentId", reason = "Hard delete removes all data including S3 files, chunks, metadata, and graph entities")
        public Mono<ResponseEntity<Object>> hardDeleteDocument(@PathVariable String documentId,
                        ServerWebExchange exchange) {
                                ...
                                .flatMap(tuple -> {
                                        ...
                                        return userService.findByUsername(username)
                                                        .flatMap(user -> teamService
                                                                        ...
                                                                        .filter(hasRole -> hasRole || user.getRoles()
                                                                                        .contains("SUPER_ADMIN"))
                                                                        .switchIfEmpty(Mono.error(
                                                                                        new AccessDeniedException(
                                                                                                        "Admin access required for team "
                                                                                                                        + teamId)))
                                                                        .then(documentService.hardDeleteDocument(
                                                                                        documentId, teamId)))
        ...
        public Mono<ResponseEntity<Object>> deleteDocument(@PathVariable String documentId,
                        ServerWebExchange exchange) {
                                                        ...
                                                        .flatMap(user -> teamService
                                                                        ...
                                                                        .switchIfEmpty(Mono.error(
                                                                                        new AccessDeniedException(
                                                                                                        "Admin access required for team "
                                                                                                                        + teamId)))
                                                                        .then(documentService.deleteDocument(documentId,
                                                                                        teamId)))
                                                        .then(Mono.just(ResponseEntity.ok().build()));
                                })
                                .onErrorResume(error -> {
        ...
        /**
         * Delete document artifacts from a specific pipeline stage (processed data,
         * metadata, embeddings)
         */
                                                        ...
                                                        .flatMap(user -> teamService
                                                                        ...
                                                                        .switchIfEmpty(Mono.error(
                                                                                        new AccessDeniedException(
                                                                                                        "Admin access required for team "
                                                                                                                        + teamId)))
                                                                        .then(documentService.deleteDocumentArtifacts(
                                                                                        documentId, teamId,
                                                                                        deletionScope)))
                                                        .then(Mono.just(ResponseEntity.ok().build()));
                                })
                                .onErrorResume(error -> {
                                        log.error("Error deleting artifacts for document {} with scope {}: {}",
                                                        documentId, scope, error.getMessage(), error);
                                        return Mono.just(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                                        .body("Failed to delete document artifacts: "
                                                                        + error.getMessage()));
                                });
        }
}
