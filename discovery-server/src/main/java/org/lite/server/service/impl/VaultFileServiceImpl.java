 ...
 * 
 * Note: discovery-server only reads from vault. Writing is done via
 * vault-reader CLI.
 */
@Service
@Slf4j
...
public class VaultFileServiceImpl implements VaultFileService {
        ...
        try {
            ...
            if (vault.getEnvironments() != null) {
                ...
                if (envSecrets != null && envSecrets.getSecrets() != null) {
                    ...
                    log.warn(
                            "Vault file loaded but environment '{}' not found or has no secrets. Available environments: {}",
    ...
    private File resolveVaultFile(String path) {
        ...
        if (vaultFile.isAbsolute()) {
            ...
            // Convert /app/secrets/vault.encrypted -> ./secrets/vault.encrypted or
            // ./secrets/vault-{env}.encrypted
            if (path.equals("/app/secrets/vault.encrypted")) {
                String projectRoot = System.getProperty("user.dir");
                String env = getCurrentEnvironment();
                log.debug("Docker path not found at: {}, trying IDE path. Project root (user.dir): {}, environment: {}",
                        path, projectRoot, env);

                // Try multiple potential IDE locations - environment-specific first
                String[] possiblePaths = {
                        projectRoot != null
                                ? new File(projectRoot, "secrets/vault-" + env + ".encrypted").getAbsolutePath()
                                : null,
                ...
                log.warn(
                        "Neither Docker nor IDE vault path found. Tried: {} and various IDE locations. Will attempt to create empty vault at: {}",
        ...
        if (path.startsWith("./")) {
            String projectRoot = System.getProperty("user.dir");
            String env = getCurrentEnvironment();
            log.debug("Resolving relative path: {} from project root: {}, environment: {}", path, projectRoot, env);
            if (projectRoot != null) {
                ...
                // Try environment-specific vault file FIRST
                String envSpecificPath = relativePath.replace("vault.encrypted", "vault-" + env + ".encrypted");
                File envSpecificFile = new File(projectRootDir, envSpecificPath);
                if (envSpecificFile.exists()) {
                    log.info("Using environment-specific IDE vault path: {}", envSpecificFile.getAbsolutePath());
                    return envSpecificFile;
                }

                // Then try generic vault file
                ...
                if (relativePath.equals("secrets/vault.encrypted")) {
                    ...
                    if (dockerPath.exists()) {
                        ...
                        log.warn(
                                "Docker path also not found at: {}. Will attempt to create empty vault at IDE path: {}",
