...
public interface AgentExecutionRepository extends ReactiveMongoRepository<AgentExecution, String> {
    ...
    // Find executions by task and institution
    @Query("{'taskId': ?0, 'input_data.institution': { '$regex': '^?1$', '$options': 'i' }}")
    Flux<AgentExecution> findByTaskIdAndInstitution(String taskId, String institution);

    @Query("{'input_data.institution': { '$regex': '^?0$', '$options': 'i' }}")
    Flux<AgentExecution> findByInstitution(String institution);
    ...
    @Query(value = "{'taskId': ?0, 'input_data.institution': { '$regex': '^?1$', '$options': 'i' }}", count = true)
    Mono<Long> countByTaskIdAndInstitution(String taskId, String institution);
    ...
    // Find execution by execution ID and institution
    @Query("{'executionId': ?0, 'input_data.institution': ?1}")
    Mono<AgentExecution> findByExecutionIdAndInstitution(String executionId, String institution);
