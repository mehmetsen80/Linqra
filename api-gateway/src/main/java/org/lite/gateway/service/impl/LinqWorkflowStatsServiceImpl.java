                ...
                execution.getResponse().getMetadata().getWorkflowMetadata().stream()
                        ...
                        .forEach(stepMetadata -> {
                            ...
                            LinqWorkflowStats.StepStats stepStats = stats.getStepStats()
                                    .computeIfAbsent(stepMetadata.getStep(), k -> new LinqWorkflowStats.StepStats());
                            ...
                            if (target != null) {
                                LinqWorkflowStats.TargetStats targetStats = stats.getTargetStats()
                                        .computeIfAbsent(target, k -> new LinqWorkflowStats.TargetStats());
                                ...
                                if (target.equals("openai-chat") || target.equals("gemini-chat")
                                        || target.equals("cohere-chat") || target.equals("claude-chat") ||
                                        target.equals("openai-embed") || target.equals("gemini-embed")
                                        || target.equals("cohere-embed") ||
                                        target.equals("ollama-chat") || target.equals("ollama-embed")) {
                                    ...
                                    if (execution.getRequest() != null &&
                                        ...
                                        model = execution.getRequest().getQuery().getWorkflow().stream()
                                                .filter(step -> step != null
                                                        && step.getStep() == stepMetadata.getStep())
                                                .findFirst()
                                                .map(step -> step.getLlmConfig() != null
                                                        ? step.getLlmConfig().getModel()
                                                        : null)
                                                .orElse("unknown");
                                    }

                                    LinqWorkflowStats.ModelStats modelStats = stats.getModelStats()
                                            .computeIfAbsent(model, k -> new LinqWorkflowStats.ModelStats());
                                    ...
                                    if (stepDuration != null) {
                                        modelStats.setAverageDurationMs(
                                                (modelStats.getAverageDurationMs()
                                                        * (modelStats.getTotalExecutions() - 1) +
                                    ...
                                    if (execution.getResponse()
                                        ...
                                        if (steps != null && steps.size() > stepMetadata.getStep()) {
                                            ...
                                            if (step.getResult() instanceof Map) {
                                                ...
                                                } else if (target.equals("cohere-chat")
                                                        || target.equals("cohere-embed")) {
                                                ...
                                                } else if (target.equals("ollama-chat")
                                                        || target.equals("ollama-embed")) {
                                                    // Ollama token usage
                                                    if (result.containsKey("prompt_eval_count")) {
                                                        promptTokens = ((Number) result.get("prompt_eval_count"))
                                                                .longValue();
                                                    }
                                                    if (result.containsKey("eval_count")) {
                                                        completionTokens = ((Number) result.get("eval_count"))
                                                                .longValue();
                                                    }
                                                    totalTokens = promptTokens + completionTokens;
                                                }
                                                ...
                                                if (totalTokens > 0) {
                                                    modelStats.setTotalPromptTokens(
                                                            modelStats.getTotalPromptTokens() + promptTokens);
                                                    modelStats.setTotalCompletionTokens(
                                                            modelStats.getTotalCompletionTokens() + completionTokens);
                                                    modelStats
                                                            .setTotalTokens(modelStats.getTotalTokens() + totalTokens);
                                                }
                                            }
                                        }
                                    }
                ...
                .flatMap(executions -> {
                    ...
                    return Flux.fromIterable(workflowIds)
                            .flatMap(workflowId -> workflowRepository.findById(workflowId)
                                    ...
                                    .switchIfEmpty(
                                            Mono.just(new AbstractMap.SimpleEntry<>(workflowId, "Unknown Workflow"))))
                                        ...
                                        execution.getResponse().getMetadata().getWorkflowMetadata().stream()
                                                ...
                                                .forEach(stepMetadata -> {
                                                    ...
                                                    TeamWorkflowStats.StepStats stepStats = stats.getStepStats()
                                                            .computeIfAbsent(String.valueOf(stepMetadata.getStep()),
                                                                    k -> {
                                                    ...
                                                    stepStats.incrementExecutions(
                                                            "success".equals(stepMetadata.getStatus()));
                                                    ...
                                                    if (execution.getRequest() != null &&
                                                            execution.getRequest().getQuery() != null &&
                                                            execution.getRequest().getQuery().getWorkflow() != null) {
                                                        intent = execution.getRequest().getQuery().getWorkflow()
                                                                .stream()
                                                                .filter(step -> step != null
                                                                        && step.getStep() == stepMetadata.getStep())
                                                                .findFirst()
                                                    ...
                                                    workflowStepStats.incrementExecutions(
                                                            "success".equals(stepMetadata.getStatus()));
                                                    ...
                                                    if (stepMetadata.getTarget().equals("openai-chat")
                                                            || stepMetadata.getTarget().equals("gemini-chat")
                                                        ...
                                                        if (execution.getRequest() != null &&
                                                                execution.getRequest().getQuery() != null &&
                                                                execution.getRequest().getQuery()
                                                                        .getWorkflow() != null) {
                                                            model = execution.getRequest().getQuery().getWorkflow()
                                                                    .stream()
                                                                    .filter(step -> step != null
                                                                            && step.getStep() == stepMetadata.getStep())
                                                                    .findFirst()
                                                        ...
                                                        if (stepMetadata.getTokenUsage() != null) {
                                                            modelStats.setTotalPromptTokens(
                                                                    modelStats.getTotalPromptTokens() + stepMetadata
                                                                            .getTokenUsage().getPromptTokens());
                                                            modelStats.setTotalCompletionTokens(
                                                                    modelStats.getTotalCompletionTokens() + stepMetadata
                                                                            .getTokenUsage().getCompletionTokens());
                                                            modelStats.setTotalTokens(modelStats.getTotalTokens()
                                                                    + stepMetadata.getTokenUsage().getTotalTokens());
                                                        }
                                                    }
                                                });
                                    }
