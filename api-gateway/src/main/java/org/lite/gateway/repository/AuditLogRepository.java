...
public interface AuditLogRepository extends ReactiveMongoRepository<AuditLog, String> {
         ...
         * Note: MongoDB reactive repositories don't support Pageable directly in method
         * signatures,
        ...
        /**
         * Count audit logs by team ID
         */
        Mono<Long> countByTeamId(String teamId);

        /**
         * Count logs ready for archival by team
         */
        @Query(value = "{ 'teamId': ?0, 'timestamp': { $lt: ?1 }, 'archivedAt': null }", count = true)
        Mono<Long> countLogsReadyForArchivalByTeam(String teamId, LocalDateTime threshold);

        /**
         * Count archived logs by team ID
         */
        Mono<Long> countByTeamIdAndArchivedAtNotNull(String teamId);

        /**
         * Find latest audit log for a team
         */
        Mono<AuditLog> findTopByTeamIdOrderByTimestampDesc(String teamId);

        /**
         * Find oldest audit log for a team
         */
        Mono<AuditLog> findTopByTeamIdOrderByTimestampAsc(String teamId);
}
