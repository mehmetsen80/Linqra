...
import org.lite.gateway.service.impl.EarlyVaultService;
 ...
 * 
 * This is the proper Spring Boot extension point for early PropertySource registration.
 */
@Slf4j
...
public class VaultEnvironmentPostProcessor implements EnvironmentPostProcessor {
    ...
    public void postProcessEnvironment(@NonNull ConfigurableEnvironment environment, SpringApplication application) {
        ...
        try {
            ...
            System.err.println("Active profiles: " + (activeProfiles.length > 0 ? String.join(", ", activeProfiles) : "none (default)"));
            ...
            if (vaultMasterKey == null || vaultMasterKey.isEmpty()) {
                ...
                // IMPORTANT: Set it as a system property so Spring Boot can resolve ${VAULT_MASTER_KEY}
                // This is needed for @Value("${vault.master.key}") which resolves to ${VAULT_MASTER_KEY}
                if (vaultMasterKey != null && !vaultMasterKey.isEmpty()) {
            ...
            log.info("Using vault file: {}, master key: {}...", vaultFilePath, vaultMasterKey.substring(0, Math.min(8, vaultMasterKey.length())));
