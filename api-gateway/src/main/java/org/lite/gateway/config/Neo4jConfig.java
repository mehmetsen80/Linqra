...
public class Neo4jConfig {
    ...
    public Driver neo4jDriver() {
        // Validate and normalize URI - Neo4j Java driver requires bolt:// protocol
        if (neo4jUri != null && neo4jUri.startsWith("http://")) {
            log.warn("Neo4j URI uses http:// scheme, converting to bolt://. Use bolt:// for the Java driver (http:// is for Browser UI)");
            neo4jUri = neo4jUri.replace("http://", "bolt://");
        } else if (neo4jUri != null && neo4jUri.startsWith("https://")) {
            log.warn("Neo4j URI uses https:// scheme, converting to bolt://. Use bolt:// for the Java driver");
            neo4jUri = neo4jUri.replace("https://", "bolt://");
        }
        
        if (neo4jUri == null || !neo4jUri.startsWith("bolt://")) {
            throw new IllegalArgumentException("Neo4j URI must use bolt:// protocol (e.g., bolt://localhost:7687). Found: " + neo4jUri);
        }
        
        log.info("Initializing Neo4j driver with URI: {}", neo4jUri);
        log.info("Neo4j username: {}, password configured: {}", neo4jUsername, neo4jPassword != null && !neo4jPassword.isEmpty());
        ...
        try {
            ...
            if (neo4jUsername == null || neo4jUsername.isEmpty() || neo4jPassword == null || neo4jPassword.isEmpty()) {
                log.error("Neo4j username or password is required but not provided. Please set NEO4J_USERNAME and NEO4J_PASSWORD environment variables");
