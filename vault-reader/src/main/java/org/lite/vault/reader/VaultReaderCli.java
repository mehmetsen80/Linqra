...
public class VaultReaderCli {
    ...
    public static void main(String[] args) {
        try {
            ...
            if (vaultFile == null || masterKey == null) {
                ...
                System.err.println("    VaultReaderCli --file <vault-file> --master-key <base64-key> [--environment <env>] [--format <env|json>] [--key <secret-key>]");
                System.err.println("  Write secret:");
                System.err.println("    VaultReaderCli --operation write --file <vault-file> --master-key <base64-key> --environment <env> --key <secret-key> --value <secret-value>");
                System.err.println("  Create empty vault:");
                System.err.println("    VaultReaderCli --operation create --file <vault-file> --master-key <base64-key>");
                System.exit(1);
            }
    ...
    private static VaultFile loadVault(String vaultFilePath, String masterKeyBase64, String environment) throws Exception {
    ...
    /**
     * Output environment variables in .env file format: VAR=value or VAR="value" (quotes only if needed)
     */
    ...
    /**
     * Helper method to output a single environment variable (reduces code duplication)
     ...
     * @param escapedValue The escaped value to output (already properly formatted for the target format)
     * @param useExportFormat If true, output "export VAR=\"value\"", else output "VAR=value" (escapedValue already formatted)
     */
    private static void outputEnvironmentVariable(Map<String, String> keyMapping, String vaultKey, 
        ...
        if (envVar != null) {
            if (useExportFormat) {
                ...
                // For env-file: escapedValue is already correctly formatted (with or without quotes)
                System.out.println(prefix + envVar + "=" + escapedValue);
            }
            
            // Special case: MongoDB username/password also map to MONGO_INITDB_ROOT_* for docker-compose
