...
/**
 * Standalone vault service for early initialization (before Spring beans are
 * created).
 * This service can decrypt and read from the vault file without requiring
 * Spring DI.
 */
@Slf4j
public class EarlyVaultService implements LinqraVaultService {
    ...
    public String getSecret(String key, String env) {
        try {
            ...
            // Let SecretNotFoundException propagate so VaultPropertySource can handle it
            // gracefully
            throw e;
        } catch (Exception e) {
    ...
    private String resolveVaultPath(String path) {
        ...
        // Try environment-specific vault in IDE
        if (envSpecificFile.exists()) {
            log.info("Using environment-specific vault: {}", envSpecificFile.getAbsolutePath());
            return envSpecificFile.getAbsolutePath();
        }

        // Fallback to non-environment-specific vault file
        File genericVaultFile = new File(projectRoot, "secrets/vault.encrypted");
        if (path.equals("/app/secrets/vault.encrypted") && genericVaultFile.exists()) {
            log.warn("Docker vault path not found, using IDE path: {}", genericVaultFile.getAbsolutePath());
            return genericVaultFile.getAbsolutePath();
        }
